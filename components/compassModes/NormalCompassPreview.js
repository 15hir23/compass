...
export default function NormalCompassPreview({ size }) {
  ...
  const getDirectionPosition = (index, isCardinal = false, dir = '') => {
    const angle = (index * 45 - 90) * (Math.PI / 180);
    // Position text well inside the circle - with padding from edge
    // Cardinal directions get more space, intercardinal get less
    // Move N up a bit more
    const textPadding = isCardinal ? (dir === 'N' ? 12 : 11) : 11;
    const r = radius - textPadding;
    return {
    ...
    <View style={[styles.container, { width: size, height: size }]}>
      <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        {DIRECTIONS.map((dir, index) => {
          const pos = getDirectionPosition(index, CARDINAL_DIRECTIONS.includes(dir), dir);
          const isCardinal = CARDINAL_DIRECTIONS.includes(dir);
          
          return (
            <G key={dir}>
              {/* Marker line - stops before text area */}
              <Line
                ...
                x2={center + (isCardinal ? radius - 18 : radius - 14) * Math.cos((pos.angle - 90) * Math.PI / 180)}
                y2={center + (isCardinal ? radius - 18 : radius - 14) * Math.sin((pos.angle - 90) * Math.PI / 180)}
                stroke={isCardinal ? "#B8860B" : "#DAA520"}
                strokeWidth={isCardinal ? 2.5 : 1.2}
              />
              
              {/* Label - positioned well inside circle */}
              <SvgText
                x={pos.x}
                y={pos.y + (isCardinal ? (dir === 'N' ? 1 : 3) : 4)}
